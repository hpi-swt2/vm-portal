...
class VmsController < ApplicationController
  ...
  def update
    notify_changed_users(@vm.sudo_users.map(&:id), info_params[:vm_info][:sudo_user_ids].map(&:to_i), true, @vm.name)
    notify_changed_users(@vm.users.map(&:id), info_params[:vm_info][:non_sudo_user_ids].map(&:to_i), false, @vm.name)
    @vm.sudo_users = info_params[:vm_info][:sudo_user_ids]
    @vm.users = info_params[:vm_info][:non_sudo_user_ids]
    @vm.config.description = info_params[:vm_info][:description]
